...
import { Button } from '.';
import { PizzaCardProps } from '../types/pizzaCard';
...
const PizzaCard: FC<PizzaCardProps> = ({
  ...
  const changeType = (index: number) => setActiveType(index);
  const changeSize = (index: number) => setActiveSize(index);
    ...
    <div className={`pizza-card ${isLoaded ? 'pizza-card__stretched' : ''}`}>
      ...
      <div className='pizza-card__buy'>
        ...
        <Button onClick={onAddPizza} className='button__add' outline>
          ...
          <span>Добавить {pizzasInCart && <b>{pizzasInCart}</b>}</span>
        </Button>
      </div>
    </div>
  );
